...
def _apply_amd_gpu_patch():
    ...
    try:
        ...
        from comfy_api._util import VideoContainer, VideoCodec

        if hasattr(video_types, 'VideoFromComponents'):
            original_save_to = video_types.VideoFromComponents.save_to

            def patched_save_to(self, path, format=VideoContainer.AUTO, codec=VideoCodec.AUTO, metadata=None):
...
# Patch is applied on-demand inside SaveAMDVideo.save_video() below.
...
class SaveAMDVideo:
    ...
    def save_video(self, video, filename_prefix, format, codec, prompt=None, extra_pnginfo=None):
        ...
        # Apply patch only when this node is actually used
