# Plan: Extract `get_input_mapping()`, `get_max_images()`, and `get_array_input_field()` Functions

## Functions Overview
**Functions**:
1. `get_input_mapping()`
2. `get_max_images()`
3. `get_array_input_field()`
...
The functions already exist in `common/config_loader.py` as part of the `ConfigLoader` class. They are used to get input mappings, max images, and array input fields from the configuration files.
...
- [ ] Verify `get_input_mapping()` returns correct mapping for API
- [ ] Verify `get_input_mapping()` returns empty dict when not configured
- [ ] Verify `get_max_images()` returns correct max images for model
- [ ] Verify `get_max_images()` returns 0 when not configured
- [ ] Verify `get_array_input_field()` returns correct field name for model
- [ ] Verify `get_array_input_field()` returns None when not configured
